...
public class IdentityProviderServletContextListener implements
        ...
        private void initProtocolServices(ServletContextEvent event) {
                ...
                for (IdentityProviderProtocolType identityProviderProtocolType :
                        ...
                        if (null != wsEndpoints) {
                                for (WSEndpointType wsEndpoint : wsEndpoints.getWSEndpoint()) {
                                        ...
                                        LOG.debug("WS Endpoint: path=" + contextPath
                                                + " impl=" + wsImplClass);
                ...
                if (!wsEndpointsMap.isEmpty()) {
                        ...
                        try {
                                wsServletClass = classLoader.loadClass(
         ...
         * @param wsEndpointsMap endpoints map
         */
        ...
        private List<ServletAdapter> getAdapters(Map<String, String> wsEndpointsMap)
                ...
                for (Map.Entry<String, String> wsEndpoint : wsEndpointsMap.entrySet()) {
                        adapters.add(getAdapter(wsEndpoint.getKey(),
                                wsEndpoint.getValue()));
                }
                return adapters;
        }
        ...
        private ServletAdapter getAdapter(String urlPattern, String wsImplClassName)
                ...
                Class<?> implementorClass = Class.forName(wsImplClassName,
                        true, classLoader);
                ...
                WSEndpoint<?> endpoint = WSEndpoint.create(
                        implementorClass, true, null, serviceName, portName,
                        container, binding, primaryWSDL, docs.values(),
         ...
         * to see if it points to any WSDL. If so, returns the {@link com.sun.xml.ws.api.server.SDDocumentSource}.
         *
         * @param implementorClass WS impl class name
        ...
        private SDDocumentSource getPrimaryWSDL(Class<?> implementorClass) {
                ...
                if (wsdlFile != null) {
                        if (!wsdlFile.startsWith(JAXWS_WSDL_DD_DIR)) {
                                LOG.warn("Ignoring wrong wsdl=" + wsdlFile + ". It should start with "
                                        + JAXWS_WSDL_DD_DIR
        ...
        /**
         * Creates {@link com.sun.xml.ws.api.server.Container} implementation that hosts the JAX-WS endpoint.
         *
         * @param context servlet context
