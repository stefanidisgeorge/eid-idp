...
public abstract class UserAgentUtil {
        ...
        public static boolean isSmartPhone(String userAgent) {
                ...
                return isIphoneIpodIpad(lowerUserAgent) ||
                        isAndroid(lowerUserAgent) ||
                        isWindowsMobile(lowerUserAgent) ||
                        isSymbian(lowerUserAgent) ||
                        isBlackberry(lowerUserAgent);
        }

        private static boolean isIphoneIpodIpad(String userAgent) {
                return isIphone(userAgent) || isIpod(userAgent)
                        || isIpad(userAgent);
        }
        ...
        private static boolean isSymbian(String userAgent) {
                return userAgent.indexOf(SYMBIAN) != -1 ||
                        userAgent.indexOf(S60) != -1 ||
                        userAgent.indexOf(S70) != -1 ||
                        userAgent.indexOf(S80) != -1 ||
